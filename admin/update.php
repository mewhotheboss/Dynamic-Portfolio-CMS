...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $sql = "UPDATE projects SET title=?, subtitle=?, para=?, url=?, image=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssi", $title, $subtitle, $para, $url, $unique_name, $id);
    
    } else {
        $sql = "UPDATE projects SET title=?, subtitle=?, para=?, url=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssi", $title, $subtitle, $para, $url, $id);
    }
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-lg-8">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="url" value="<?php echo htmlspecialchars($project['url']); ?>" required>
                            </div>
                        </div>
                    </div>
