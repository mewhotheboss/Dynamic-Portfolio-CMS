...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE about SET card_title=?, card_para=?, title=?, para=?, resume=? WHERE id=1";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $card_title, $card_para, $title, $para, $resume);
...
if ($result->num_rows == 0) {
    $conn->query("INSERT INTO about (id, card_title, card_para, title, para, resume) VALUES (1, 'My Role', 'Short intro...', 'Main Title', 'Long bio...', 'Resume link or #(default)')");
    $result = $conn->query("SELECT * FROM about WHERE id=1");
}
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="resume" value="<?php echo htmlspecialchars($about['resume']); ?>" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Card Intro Text (Short)</label>
                                <textarea class="form-control" name="card_para" rows="5" required><?php echo htmlspecialchars($about['card_para']); ?></textarea>
                            </div>
                        </div>
                    </div>
