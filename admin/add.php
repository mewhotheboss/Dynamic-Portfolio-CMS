...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {    
    ...
    if(move_uploaded_file($tempname, $folder)){
        ...
        $sql = "INSERT INTO projects (title, subtitle, para, url, image) VALUES (?,?,?,?,?)";
        if($stmt = $conn->prepare($sql)){
            $stmt->bind_param("sssss", $title, $subtitle, $para, $url, $db_image);
            $stmt->execute();
            $message = "success";
            $stmt->close();
        } else {
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 col-lg-8">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="para" rows="6" placeholder="Enter project details..." required></textarea>
                            </div>
                            <div class="form-group">
                                <label for="url">Project Url</label>
                                <input type="text" class="form-control" name="url" placeholder="e.g. www.example.com or write '#'(default)" required>
                            </div>
                        </div>
                    </div>
